...
def get_url():
    ...
    excel_file = 'YuddhaKanda.xlsx'
    df = pd.read_excel(excel_file,header=3,usecols=['Verses'])
    values = df.values.tolist()
    number_of_verses = [item for sublist in values for item in sublist]
    #print(number_of_verses)

    sarga_number = 1
    number_of_chapters = 128
    j = 0
    ...
    file = open("yuddhakanda.txt","w",encoding="utf-8")

    while number_of_chapters >= sarga_number:
        url = requests.get(f"http://www.valmikiramayan.net/utf8/yuddha/sarga{sarga_number}/yuddhasans{sarga_number}.htm")
    ...
    wc.to_file('wordcloud_yuddha.png')
